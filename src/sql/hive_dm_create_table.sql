CREATE TABLE `dm_user_basic` (
  `user_id` bigint,
  `user_name` string,
  `user_gender` string,
  `user_birthday` string,
  `user_age` bigint,
  `constellation` string,
  `province` string,
  `city` string,
  `city_level` int,
  `e_mail` string,
  `op_mail` string,
  `mobile` bigint,
  `num_seg_mobile` int,
  `op_mobile` string,
  `register_time` datetime,
  `login_ip` string,
  `login_source` string,
  `request_user` string,
  `total_score` decimal,
  `used_score` decimal,
  `is_blacklist` tinyint,
  `is_married` tinyint,
  `education` string,
  `monthly_income` decimal,
  `profession` string,
  `is_pregnant_woman` int,
  `is_have_children` int,
  `is_have_car` int,
  `phone_brand` string,
  `phone_brand_level` string,
  `phone_cnt` int,
  `change_phone_cnt` int,
  `is_maja` int,
  `majia_account_cnt` int,
  `loyal_model` string,
  `shopping_type_model` string,
  `weight` int,
  `height` int,
  `dw_date` string
);

CREATE TABLE qfbap_dm.dm_user_visit (
`user_id` bigint,
`latest_pc_visit_date` string,
`latest_pc_visit_session` string,
`latest_pc_cookies` string,
`latest_pc_pv` bigint,
`latest_pc_browser_name` string,
`latest_pc_visit_os` string,
`first_pc_visit_date` string,
`first_pc_visit_session` string,
`first_pc_cookies` string,
`first_pc_pv` bigint,
`first_pc_browser_name` string,
`first_pc_visit_os` string,
`day7_pc_cnt` bigint,
`day15_pc_cnt` bigint,
`month1_pc_cnt` bigint,
`month2_pc_cnt` bigint,
`month3_pc_cnt` bigint,
`month1_pc_days` bigint,
`month1_pc_pv` bigint,
`month1_pc_avg_pv` bigint,
`month1_pc_hour025_cnt` bigint,
`month1_pc_hour627_cnt` bigint,
`month1_pc_hour829_cnt` bigint,
`month1_pc_hour10211_cnt` bigint,
`month1_pc_hour12213_cnt` bigint,
`month1_pc_hour14216_cnt` bigint,
`month1_pc_hour17219_cnt` bigint,
`month1_pc_hour18219_cnt` bigint,
`month1_pc_hour20221_cnt` bigint,
`month1_pc_hour22223_cnt` bigint,
`month1_pc_diff_ip_cnt` bigint,
`month1_pc_common_ip` string,
`month1_pc_diff_cookie_cnt` bigint,
`month1_pc_common_cookie` string,
`month1_pc_common_browser_name` string,
`month1_pc_common_os` string,
`latest_app_visit_date` string,
`latest_app_name` string,
`latest_app_visit_os` string,
`first_app_visit_date` string,
`first_app_name` string,
`first_app_visit_os` string,
`day7_app_cnt` bigint,
`day15_app_cnt` bigint,
`month1_app_cnt` bigint,
`month2_app_cnt` bigint,
`month3_app_cnt` bigint,
`month1_app_hour025_cnt` bigint,
`month1_app_hour627_cnt` bigint,
`month1_app_hour829_cnt` bigint,
`month1_app_hour10211_cnt` bigint,
`month1_app_hour12213_cnt` bigint,
`month1_app_hour14215_cnt` bigint,
`month1_app_hour16217_cnt` bigint,
`month1_app_hour18219_cnt` bigint,
`month1_app_hour20221_cnt` bigint,
`month1_app_hour22223_cnt` bigint,
`latest_visit_ip` string,
`latest_city` string,
`latest_province` string,
`first_visit_ip` string,
`first_city` string,
`first_province` string
);